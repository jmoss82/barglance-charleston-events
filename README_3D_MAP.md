...
5. **Set environment variables:** In **Settings → Environment Variables** add:
   ...
   - `BLOB_READ_WRITE_TOKEN` � Vercel Blob token (used by `api/upload-image.js`)
...
- `image-overrides.json` - Optional per-venue image/crop overrides consumed by the display
- `image_admin.html` - Admin page to edit image overrides and export `image-overrides.json`
...
- `api/upload-image.js` - Uploads admin-selected image files to Vercel Blob and returns a public URL
...
3. Bars API `images.hero`
4. Event `bar.profile_picture_url` fallback
